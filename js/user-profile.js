document.addEventListener("DOMContentLoaded", function () {
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          document.getElementById("join-date").textContent =
            formatDate(userData.joinDate) || "New Member";
  ...
  // Load modules progress from Firestore
  function loadModulesProgress(userId) {
    // Implement this to fetch progress data from Firestore
    // For now, we're using the static data in the HTML
  }

  // Load badges from Firestore
  function loadBadges(userId) {
    // Implement this to fetch badges data from Firestore
    // For now, we're using the static data in the HTML
  }
  ...
  saveProfileBtn.addEventListener("click", function () {
    ...
    if (firebase && firebase.auth && firebase.auth().currentUser) {
      ...
      console.log("Changes would be saved to Firebase (mock)");
      showNotification("Profile updated successfully", "success");
    }
  ...
  passwordForm.addEventListener("submit", function (e) {
    ...
    if (firebase && firebase.auth && firebase.auth().currentUser) {
      ...
      // For testing when Firebase is not available
      console.log("Password would be updated in Firebase (mock)");
      showNotification("Password updated successfully", "success");
      passwordForm.reset();
    }
  });
  ...
  if (resetProgressBtn) {
    resetProgressBtn.addEventListener("click", function () {
      if (
        ...
        // Reset progress in Firebase if available
        if (firebase && firebase.auth && firebase.auth().currentUser) {
          const userId = firebase.auth().currentUser.uid;
          ...
          // For testing when Firebase is not available
          console.log("Progress would be reset in Firebase (mock)");
          showNotification("Progress has been reset", "warning");
    ...
    deleteAccountBtn.addEventListener("click", function () {
      if (
        ...
        if (
          ...
          // Delete account in Firebase if available
          if (firebase && firebase.auth && firebase.auth().currentUser) {
            const user = firebase.auth().currentUser;
            const userId = user.uid;
